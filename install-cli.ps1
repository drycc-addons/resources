...
<#
.SYNOPSIS
    Install Drycc Resources CLI for Windows.

.DESCRIPTION
    This script downloads and installs the Drycc Resources CLI (drycc-resources.exe)
    to the specified directory.

    Supports one-liner installation via:
        irm https://raw.githubusercontent.com/drycc-addons/resources/main/install-cli.ps1 | iex
...
.PARAMETER InstallPath
    The directory to install drycc-resources.exe. Defaults to $env:LOCALAPPDATA\drycc.

.PARAMETER Version
    The version of drycc-resources to install. Defaults to 'stable'.
...
.EXAMPLE
    $env:INSTALL_DRYCC_MIRROR="cn"; irm https://raw.githubusercontent.com/drycc-addons/resources/main/install-cli.ps1 | iex
#>
...
if ($env:INSTALL_DRYCC_MIRROR -eq "cn") {
    $script:DryccBinUrlBase = "https://drycc-mirrors.drycc.cc/drycc-addons/resources/releases"
} else {
    # FIX: The else branch was incorrectly pointing to the CN mirror.
    # It should use the original GitHub releases URL.
    $script:DryccBinUrlBase = "https://github.com/drycc-addons/resources/releases"
}
...
function Get-LatestVersion {
    ...
    $version = $content |
        Select-String -Pattern '/drycc-addons/resources/releases/tag/(v[0-9\.]+)' |
...
function Install-DryccCli {
    ...
    # FIX #2: A previous "fix" incorrectly removed the .exe suffix from the
    # download URL. The release assets in THIS repository (drycc-addons/resources)
    # DO include a .exe extension for Windows binaries. Removing it causes a
    # 404 Not Found. We must preserve .exe in the asset name.
    $assetName = "drycc-resources-${Version}-${platform}-${arch}.exe"
    $downloadUrl = "$script:DryccBinUrlBase/download/${Version}/${assetName}"
    $tempFile = [System.IO.Path]::GetTempFileName() + ".exe"

    Write-Host "Downloading Drycc Resources CLI ${Version} for ${platform}-${arch}..."
    ...
    $installExe = Join-Path $InstallPath "drycc-resources.exe"
    ...
    Write-Host ""
    Write-Host "Drycc Resources CLI (drycc-resources) has been installed to: $installExe"
    Write-Host ""
    Write-Host "To learn more about Drycc Resources, execute:"
