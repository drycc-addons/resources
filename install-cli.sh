...
if [[ "${INSTALL_DRYCC_MIRROR}" == "cn" ]] ; then
    DRYCC_BIN_URL_BASE="https://drycc-mirrors.drycc.cc/drycc-addons/resources/releases"
else
    DRYCC_BIN_URL_BASE="https://github.com/drycc-addons/resources/releases"
fi
...
The Drycc Resources CLI (drycc-resources) is not currently supported on ${PLATFORM}-${ARCH}.

See https://github.com/drycc-addons/resources for more information.
...
init_latest_version() {
  VERSION=$(curl -Ls $DRYCC_BIN_URL_BASE|grep /drycc-addons/resources/releases/tag/ | sed -E 's/.*\/drycc-addons\/resources\/releases\/tag\/(v[0-9\.]+)".*/\1/g' | head -1)
  if [ -z "${VERSION}" ]; then
    echo "Error: unable to determine the latest drycc-resources version from GitHub API." >&2
...
DRYCC_CLI="drycc-resources-${VERSION}-${PLATFORM}-${ARCH}"
...
curl -fsSL -o "${INSTALL_DIR}/drycc-resources" "${DRYCC_BIN_URL_BASE}/download/${DRYCC_CLI_PATH}" < /dev/null
chmod +x "${INSTALL_DIR}/drycc-resources"

echo ""
echo "The Drycc Resources CLI (drycc-resources) has been installed to: ${INSTALL_DIR}/drycc-resources"
...
if [ "${PATH_OK}" -eq 1 ]; then
  echo ""
  echo "To learn more about Drycc Resources, execute:"
  echo ""
  echo "    $ drycc-resources --help"
  echo ""
  exit 0
fi
...
echo ""
echo "If 'drycc-resources' is still not found, add it to PATH manually:"
echo ""
